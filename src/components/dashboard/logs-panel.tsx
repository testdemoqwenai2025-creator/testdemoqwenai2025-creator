...
export function LogsPanel({ logs }: LogsPanelProps) {
  ...
  const agents = useMemo(() => {
    const s = new Set(logs.map((l) => l.agent))
  ...
  function formatTimestamp(ts: string): string {
    const d = new Date(ts)
    return `${d.getUTCHours().toString().padStart(2, '0')}:${d.getUTCMinutes().toString().padStart(2, '0')}:${d.getUTCSeconds().toString().padStart(2, '0')}.${d.getUTCMilliseconds().toString().padStart(3, '0')} UTC`
  }
    ...
    <Card>
      ...
      <CardContent>
        <div className="text-xs text-muted-foreground mb-2">
          Showing {filtered.length} of {logs.length} agent events
        </div>
            ...
            {filtered.map((log, i) => (
              ...
              >
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  <Badge className={`text-[10px] h-5 shrink-0 ${AGENT_COLORS[log.agent] || ''}`}>
                    {log.agent.replace('_', ' ')}
                  </Badge>
                  {log.fields.jurisdiction && (
                    <Badge variant="outline" className="text-[10px] h-5 shrink-0">{log.fields.jurisdiction}</Badge>
                  )}
                  {log.fields.alerting && (
                ...
                <div className="flex items-center gap-3 text-[10px] text-muted-foreground">
                  ...
                  <span>role: {log.fields.agent_role}</span>
                  <span>reg: {log.fields.regulation_id}</span>
                </div>
              </div>
            ))}
          </div>
